function getArticleTemplate(name, quantity) {
    return `
         <tr class="article">
            ...
            <td>`+quantity+`</td>
            <td>
                <span><a type="button" onclick="deleteArticleRequestSale(this)" class="text-danger"><i
...
function addNewArticleQuantitySale() {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://localhost:8080/SystemeCommerciale/proforma-sending',
        data: {
...
function deleteArticleRequestSale(bouton) {
    ...
    $.ajax({
        type: 'GET',
        url: 'http://localhost:8080/SystemeCommerciale/DeleteProformaSending',
        data: {
            code: id
        },
